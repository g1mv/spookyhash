...
#include "cputime/src/cputime_api.h"
...
uint64_t test_value() {
    uint64_t e = m_a - SPOOKYHASH_ROTATE(m_b, 23);
    m_a = m_b ^ SPOOKYHASH_ROTATE(m_c, 16);
    m_b = m_c + SPOOKYHASH_ROTATE(m_d, 11);
    m_c = m_d + e;
    m_d = e + m_a;
    return m_d;
}
...
void test_add(const void *data, size_t length, uint64_t *hash1, uint64_t *hash2) {
